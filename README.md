...
Document Organizer is a secure, local-first document management system designed to organize, index, and archive local files. It utilizes a hybrid desktop client-server architecture to provide a native app experience while leveraging local system resources.

---

## Supported Platforms & Devices

Document Organizer is designed to be highly portable and compatible across a range of desktop platforms and hardware architectures:
...
Document Organizer is architected as a **local-first standalone desktop application** to guarantee privacy, performance, and low-latency offline document manipulation. However, its modular architecture makes it flexible:
...
To run or build Document Organizer locally, ensure you have the following installed:
...
The backend creates and updates the SQLite database `file_metadata.db` locally in the `.documentorganizer/` folder in the user's home directory.
```bash
cd backend
mvn spring-boot:run
```
